 ...
 *
 * CRITICAL RULES (do not break left HUD / struggle panel):
 *  - Never set item.type (inventory category)
 *  - Never set item.Model on the live item
 *  - Never rekey KinkyDungeonInventory Map (desyncs restraint UI)
 *  - Never return null from KDDrawBuffIcons
 *  - Never call CharacterRefresh / DressPlayer synchronously in input path
 *  - Appearance = KDRestraint(item).Model from restraint def after name change
 */
"use strict";
...
var RG_OPEN_TEXT: any = {
	PanelPlugGagHarness: {
		...
		desc: "A harness gag with the plug removed.",
		desc2: "The harness still holds your jaws wide. Drool and noise escape freely.",
	},
	PanelPlugGag: {
		title: "Panel Plug Gag (Open)",
		desc: "A panel gag with the center plug removed.",
		desc2: "Without the plug, the panel frames an open mouth.",
	},
	MaidMuzzle: {
		title: "Maid Muzzle (Open)",
		desc: "A maid-styled muzzle with the plug taken out.",
		desc2: "The frills remain, but nothing seals the mouth.",
	},
	CableGag: {
		title: "Cable Gag (Open)",
		desc: "Cable straps around an open panel — plug removed.",
		desc2: "The cable still bites, but the mouth stays open.",
	},
	NylonCableGag: {
	...
	CyberPlugGag: {
		...
		desc2: "Systems report an open airway.",
	},
	GoodGirlGag: {
	...
	SteelMuzzleGag: {
		...
		desc2: "Cold metal frames an open mouth.",
	},
	BlacksteelMuzzleGag: {
...
var RG_DEBUFF_TEXT =
	"Your mouth is stretched open! Noise and drool escape with nothing to stop them...";
...
function RG_IsSwapPair(item) {
	...
	return (
		RG_SWAP_PAIRS[item.name] !== undefined ||
		RG_SWAP_PAIRS_REVERSE[item.name] !== undefined
	);
}
...
/** Soft, deferred refresh — never runs CharacterRefresh in the input stack. */
function RG_ScheduleRefresh() {
	...
	try {
		if (
			...
			KDRefreshCharacter &&
			KDRefreshCharacter.set
		)
			KDRefreshCharacter.set(KinkyDungeonPlayer, true);
	} catch (_e1) {}
	// Defer struggle-group rebuild so the current click/draw finishes first
...
function RG_RegisterOpenVariantText(baseName, openName) {
	...
	var desc2 = pack
		? pack.desc2
		: "The plug has been removed; the mouth stays open.";
	try {
		if (typeof TextGet === "function") {
			...
			if (
				s &&
				s.indexOf("[NotFound]") < 0 &&
				s.indexOf("Restraint" + baseName) !== 0
			)
				title = s + " (Open)";
		}
	} catch (_e0) {}
...
/**
 * In-place swap. Mutates ONLY:
 *   item.name, item.events
 * Does NOT touch: item.type, item.Model, inventory Map, locks (avoid event storms).
 */
function RG_PerformSwap(item) {
	...
	// Ensure open def points at the open model (definition only)
	...
	// --- the only live-item mutation ---
	item.name = siblingName;

	// Strip accidental Model override left by older builds
	try {
		var anyItem: any = item;
		if ("Model" in anyItem) delete anyItem.Model;
		if ("type" in anyItem && typeof anyItem.type === "string") {
			// type must stay an inventory category (e.g. "restraint"), never a model name
			var bad =
				anyItem.type.indexOf("Model") >= 0 ||
...
function RG_AddOpenVariant(baseName, modelOverride) {
	...
	if (!existing) {
		...
		copy.inventory = true;
		copy.weight = 0;
		// Do NOT set inventoryAs — it forces tooltips to base keys and breaks open text
		...
		copy.events =
			typeof RingGagEvents !== "undefined" && RingGagEvents
				? RingGagEvents.slice()
				: copy.events || [];
...
function RG_DoPlayerPlugSwap(item) {
	...
	try {
		var msg = wasPlugged
			? RG_Pick(RG_MSG_PLAYER_UNPLUG)
			: RG_Pick(RG_MSG_PLAYER_PLUG);
...
function RG_RegisterPlugSwap() {
	...
	if (typeof KDInventoryAction !== "undefined" && !KDInventoryAction["PlugSwap"]) {
		KDInventoryAction["PlugSwap"] = {
			...
			icon: function (_p, item) {
				return RG_IsPluggedVariant(item.name)
					? "InventoryAction/Unplug"
					: "InventoryAction/Plug";
			},
			...
			valid: function (_p, item) {
				try {
					if (
						typeof KinkyDungeonIsArmsBound === "function" &&
						KinkyDungeonIsArmsBound()
					)
						return false;
					if (
						typeof KinkyDungeonIsHandsBound === "function" &&
						KinkyDungeonIsHandsBound()
					)
						return false;
					return true;
				} catch (_e) {
			...
			click: function (_p, item) {
				try {
					...
					var sg =
						typeof KinkyDungeonStruggleGroups !== "undefined" &&
						KinkyDungeonStruggleGroups
							? KinkyDungeonStruggleGroups.find(function (g) {
									return r.Group === g.group;
							  })
							: null;
					if (!sg) {
						RG_DoPlayerPlugSwap(item);
					...
					var itemIndex =
						typeof KDGetItemLinkIndex === "function"
							? KDGetItemLinkIndex(item, false)
							: 0;
					if (itemIndex < 0) itemIndex = 0;
					if (typeof KDSendInput === "function")
						KDSendInput("plugSwap", {
							group: sg.group,
							index: itemIndex,
						});
		...
		KDInputTypes["plugSwap"] = function (data: any): string {
			try {
				...
				if (data.index && typeof KDDynamicLinkListSurface === "function") {
					...
					if (surfaceItems && surfaceItems[data.index])
	...
	// Inventory drawer: append PlugSwap safely (must always return array)
	if (
		typeof KDInventoryActionsDefault !== "undefined" &&
		KDInventoryActionsDefault.restraint &&
		!RG_InventoryActionsWrapped
	) {
		var orig = KDInventoryActionsDefault.restraint;
		KDInventoryActionsDefault.restraint = function (item) {
			var ret: any;
			try {
				ret = orig.call(this, item);
			} catch (_e) {
				ret = [];
			}
...
/**
 * Buff icon hook — MUST always call original and return its result.
 * Returning null blanks the stats / left HUD.
 */
function RG_RegisterOpenGagDebuff() {
	...
	g.KDDrawBuffIcons = function (minXX, minYY, statsDraw, side) {
		try {
			if (
				statsDraw &&
				typeof RG_HasOnlyOpenGags === "function" &&
				RG_HasOnlyOpenGags()
			) {
				statsDraw.rg_opengag = {
					...
					icon: "opengag_debuff",
					color:
						typeof KDBaseRed !== "undefined" ? KDBaseRed : "#ff4444",
					bgcolor: "#333333",
					priority: 6,
				};
			}
		} catch (_e) {}
		// Always forward — never return null
